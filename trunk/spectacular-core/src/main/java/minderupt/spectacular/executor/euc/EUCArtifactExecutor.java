...
public class EUCArtifactExecutor implements ArtifactExecutor {
    ...
    private SeleneseIndexer seleneseIndexer;
    ...
    public EUCArtifactExecutor() {
        ...
        seleneseIndexer = new SeleneseIndexer();
    ...
    public void setBasePackages(List<String> basePackageList) {
        ...
        for (String packageName : basePackageList) {
            ...
            } else if (packageName.indexOf("selenese:") == 0) {

                // remove selenese: metatag
                String scriptName = packageName.substring(9);
                if (LOGGER.isInfoEnabled()) LOGGER.info("Indexing SELENESE package for EUC:  " + scriptName);
                List<String> scriptList = new LinkedList<String>();
                scriptList.add(scriptName);
                this.seleneseIndexer.indexScripts(scriptList, flowMap, expectationMap);                                
        ...
        for (int i = 1; i < data.getRowCount(); i++) {
            ...
            for (int c = 0; c < subValueList.size(); c++) {
                ...
                if (LOGGER.isDebugEnabled())
                    LOGGER.debug("Associating var name (" + varName + ") with var value (" + varValue + ")");
    ...
    private ArtifactExecutionResults executeSingleArtifact(final GlobalOptions globalOptions, Artifact artifact) {
        ...
        context.put(Context.GLOBAL_OPTIONS_KEY, globalOptions);
